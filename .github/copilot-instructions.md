...
このリポジトリは、Excel ファイルを中間モデルに変換し、レイアウト計算と描画コマンド生成を経て PDF を出力する .NET 10 ライブラリです。実装の対象は `src/ReportEngine`、テストは `tests/ReportEngine.Tests` にあります。
...
- テストには xUnit を使用し、振る舞いを変更する場合は `tests/ReportEngine.Tests` に対応するテストを追加または更新する。
