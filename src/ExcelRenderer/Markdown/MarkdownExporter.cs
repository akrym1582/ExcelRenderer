...
public sealed class MarkdownExporter
{
    ...
    public async Task ExportAsync(
        ...
        string documentName = "workbook.xlsx",
        CancellationToken cancellationToken = default)
    {
        ...
        {
            Directory.CreateDirectory(Path.Combine(outputDirectory, options.ImageDirectoryName));
        }
        ...
        using var writer = new StreamWriter(
            Path.Combine(outputDirectory, SanitizeFileName(outputName)),
            false,
    ...
    private static string Build(ReportDocument document, string directory, string name, MarkdownExportOptions options)
    {
        ...
        foreach (var sheet in document.Sheets)
        {
            ...
            if (options.IncludeHiddenLayoutMetadata)
            {
                WriteHiddenMetadata(output, sheet);
            }
    ...
    private static void WriteRegion(StringBuilder output, SheetRegion region, MarkdownExportOptions options)
    {
        ...
        if (hasMerges)
        {
            WriteHtmlTable(output, cells, options);
        }
        else if (rectangular)
        {
            WriteMarkdownTable(output, rows, options);
        }
        else
        {
            WriteRangeList(output, cells, options);
        }
    }
    ...
    private static void WriteHtmlTable(
        StringBuilder output,
        IReadOnlyList<VisualCell> cells,
        MarkdownExportOptions options)
    {
        ...
        for (var row = firstRow; row <= lastRow; row++)
        {
            ...
            for (var column = firstColumn; column <= lastColumn;)
            {
                ...
                if (rowSpan > 1)
                {
                    output.Append(" rowspan=\"").Append(rowSpan).Append('"');
                }
    ...
    private static void WriteRangeList(
        StringBuilder output,
        IEnumerable<VisualCell> cells,
        MarkdownExportOptions options)
    {
        if (!options.IncludeCellAddresses)
        {
        ...
        MarkdownExportOptions options)
    {
        ...
        for (var i = 0; i < images.Count; i++)
        {
            ...
            if (options.IncludeImageMetadata)
            {
                ...
                {
                    output.Append("- region: ").AppendLine(Range(region.BoundingRange));
                }
            }
    ...
    private static void WriteHiddenMetadata(StringBuilder output, ReportSheet sheet)
    {
        ...
        if (rows.Length > 0)
        {
            output.Append("- Hidden rows: ").AppendLine(string.Join(", ", rows));
        }

        if (columns.Length > 0)
        {
            output.Append("- Hidden columns: ").AppendLine(string.Join(", ", columns));
        }
    ...
    {
        if (!string.IsNullOrWhiteSpace(image.Extension))
        {
            return image.Extension!.TrimStart('.').ToLowerInvariant();
        }
        ...
        if (b.Length >= 6 && Encoding.ASCII.GetString(b, 0, 3) == "GIF")
        {
            return "gif";
        }
